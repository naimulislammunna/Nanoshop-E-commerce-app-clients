...
const FilterBar = ({setBrandValue, setCategoryValue, products}) => {
        ...
        <div>
            <div>
                ...
                <div className={`${selectOpenBrand ? 'visible top-0 opacity-100' : 'hidden -top-4 opacity-0'} relative mx-auto my-4 w-full rounded-xl py-4 border duration-300`}>
                    {products.brandList?.map((option, idx) => (
                        <div key={idx} onClick={(e) => {setBrandValue(e.target.textContent); setSelectOpenBrand(false); setBrand(e.target.textContent)}} className="px-6 py-2 text-gray-500 hover:bg-gray-100">
                            {option}
                        </div>
                    ))}
                </div>
            ...
            <div>
                ...
                <div className={`${selectOpenCategory ? 'visible top-0 opacity-100' : 'hidden -top-4 opacity-0'} relative mx-auto my-4 w-full rounded-xl py-4 border duration-300`}>
                    {products.categoryList?.map((option, idx) => (
                        <div key={idx} onClick={(e) => { setCategoryValue(e.target.textContent); setSelectOpenCategory(false); setCategory(e.target.textContent)}} className="px-6 py-2 text-gray-500 hover:bg-gray-100">
                            {option}
                        </div>
                    ))}
                </div>
